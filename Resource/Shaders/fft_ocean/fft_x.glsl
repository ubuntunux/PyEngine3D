...
{
    vec4 input1 = texture2DArray(imgSampler, vec3(i.x, uv.y, layer), 0.0);
    vec4 input2 = texture2DArray(imgSampler, vec3(i.y, uv.y, layer), 0.0);
...
void main()
{
    vec4 data = texture2D(butterflySampler, vec2(uv.x, pass), 0.0);
    vec2 i = data.xy;
    vec2 w = data.zw;
