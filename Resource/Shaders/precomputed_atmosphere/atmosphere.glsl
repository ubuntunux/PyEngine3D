...
void main()
{
    ...
    {
        radiance += transmittance * GetSolarRadiance(ATMOSPHERE) * pow(clamp((VdotL - sun_size.y) / (1.0 - sun_size.y), 0.0, 1.0), 4.0);
    }
    ...
    // in-scatter & light shaft
    vec3 view_point_inscatter = vec3(0.0);
    vec3 view_point_sun_irradiance = vec3(0.0);
    vec3 view_point_sky_irradiance = vec3(0.0);

    vec3 scene_inscatter = vec3(0.0);
    ...
    GetCloudRadiance(ATMOSPHERE, NEAR_FAR.x, screen_center_ray, N, scene_shadow_length,
        view_point_sun_irradiance, view_point_sky_irradiance, view_point_inscatter);

    GetCloudRadiance(ATMOSPHERE, scene_dist, eye_direction, N, scene_shadow_length,
        scene_sun_irradiance, scene_sky_irradiance, scene_inscatter);
