...
void main() {
    ...
    vec4 color = texture(texture_source, texcoord + vec2(inv_texture_size.x, 0.0));
    float luminance = get_luminance(color.xyz);
    fs_output = mix(fs_output, color, clamp(luminance, 0.0, 1.0));

    color = texture(texture_source, texcoord + vec2(0.0, inv_texture_size.y));
    luminance = get_luminance(color.xyz);
    fs_output = mix(fs_output, color, clamp(luminance, 0.0, 1.0));

    color = texture(texture_source, texcoord + inv_texture_size);
    luminance = get_luminance(color.xyz);
    fs_output = mix(fs_output, color, clamp(luminance, 0.0, 1.0));
}
